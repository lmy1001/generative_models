...
import datasets
import os
from Autoencoder import Encoder, Decoder, Autoencoder, get_loss, save
...
net = Autoencoder(args.input_dim, args.latent_dim, [64, 128, 128, 256, args.latent_dim], [256, 256, args.npoints * 3])
...
for epoch in range(start_epoch, args.nepoch):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        output, loss, cd, emd = net(input, optimizer, step, args.loss_type, args.accelerated_cd, writer_train)
        output = output.cuda()
        ...
        if i % 10 == 0:
            ...
            vis_output = output[0]
            vis.scatter(vis_input, name="input")
            vis.scatter(vis_output, name='gen_out')
            ...
            with torch.no_grad():
                ...
                val_input_t = torch.transpose(val_input, 1, 2)
                val_output, val_loss, val_cd, val_emd = net(val_input_t, optimizer,
                                                            step, args.loss_type,
                                                            args.accelerated_cd, writer_train=None)
...
save(net, optimizer, epoch + 1,  '%s/cls_last_model_%d.pth' % (args.outf, epoch))
...
for i, data in enumerate(test_loader, 0):
    ...
    with torch.no_grad():
        ...
        test_output, test_loss, test_cd, test_emd = net(test_input_t, optimizer,
                                                    step, args.loss_type,
                                                    args.accelerated_cd, writer_train=None)
